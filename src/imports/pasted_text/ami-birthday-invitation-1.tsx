...
export default function AmiBirthdayInvitation() {
  ...
  const dresscodeImage = 'PASTE_DRESSCODE_IMAGE_LINK_HERE'

  const memories = [
    {
      image: 'PASTE_IMAGE_LINK_HERE_1',
    },
    {
      image: 'PASTE_IMAGE_LINK_HERE_2',
    },
    {
      image: 'PASTE_IMAGE_LINK_HERE_3',
    },
  ]
      ...
      <section className="py-10">
        ...
        <div className="flex gap-5 overflow-x-auto px-6 pb-4 snap-x snap-mandatory scrollbar-hide">
          {memories.map((item, index) => (
            <div
              key={index}
              className="min-w-[280px] bg-white rounded-[28px] p-3 snap-center shadow-[0_10px_30px_rgba(255,112,174,0.12)]"
            >
              <div className="aspect-[4/5] rounded-[20px] overflow-hidden border-2 border-dashed border-[#ff89bc] relative bg-[#ffeff6]">
                {item.image.includes('PASTE') ? (
                  <div className="w-full h-full flex flex-col items-center justify-center text-center p-6 bg-gradient-to-b from-[#ffbcd9] to-[#ffeff6]">
                    <div className="w-20 h-20 rounded-full bg-[#ffd6e7] flex items-center justify-center text-3xl shadow-inner mb-5">
                      📸
                    </div>
